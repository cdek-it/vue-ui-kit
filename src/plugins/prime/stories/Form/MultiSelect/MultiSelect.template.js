...
export const Template = (args) => ({
  ...
  setup() {
    ...
    return { args, value, options, inputProps };
  },
  template: `
    <MultiSelect
      v-model="value"
      ...
      optionLabel="name"
      v-bind="inputProps"
      :class="{ 'p-multiselect-xlg': args.size === 'xlarge' }"
      :size="args.size === 'xlarge' || args.size === 'medium' ? null : args.size"
      :pt="{
        filterContainer: { class: 'p-iconfield p-multiselect-filter-container' },
        filterInput: { class: 'p-inputtext-sm' },
        pcFilter: { root: { class: 'p-inputtext-sm' } },
        filterIconContainer: { class: 'p-inputicon' }
      }"
    >
      <template #filtericon>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" class="p-icon" aria-hidden="true" data-pc-section="filtericon">
...
export const TemplateFloatLabel = (args) => ({
  components: { MultiSelect, FloatLabel },
  setup() {
    const value = ref([]);
  ...
  template: `
    <FloatLabel variant="in">
      ...
      <label for="ms_label">{{ args.label }}</label>
    </FloatLabel>
  `,
});
