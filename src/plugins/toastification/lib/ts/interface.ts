...
const createToastificationApp = (globalOptions: PluginOptions) => {
  const isMounted = document?.getElementById('toastification');
  ...
  if (!isMounted) {
    ...
    root.setAttribute('id', 'toastification');
...
export const buildInterface = (
  ...
  if (mountContainer) {
    nextTick(() => {
      createToastificationApp(globalOptions);
    });
  }
  /**
   * Display a toast
