...
# Test script with old files cleanup
set -e
...
echo -e "${CYAN}Testing reorganization with cleanup for: ${COMPONENT_NAME}${NC}\n"
...
if [ -f "$old_mdx" ]; then
    echo -e "${YELLOW}Moving old .mdx file to .old${NC}"
    mv "$old_mdx" "${old_mdx}.old"
fi

if [ -f "$old_template" ]; then
    echo -e "${YELLOW}Moving old .template.js file to .old${NC}"
...
echo -e "\n${BLUE}Extracting sections from documentation...${NC}\n"
...
while IFS= read -r line; do
    if [[ "$line" =~ ^##[[:space:]](.+)$ ]]; then
        ...
        if [[ ! "$clean_name" =~ ^(Props|Emits|Templates|PassThroughOptions|PassThrough|Theming|CSSClasses|DesignTokens|Style|styledoc|Accessibility|AccessibilityDoc|Button)$ ]]; then
...
echo -e "\n${CYAN}Files in component directory:${NC}"
ls -la "$component_dir" | grep -v "^d" | tail -n +2
